...
// 当从事件链表中取出第一个结点 且第一个结点的第二个数据域为-1时（头结点的后一个结点 -1表示该客户到来） 需要随机生成两个数//
// 第一个数是为该客户服务的时间 第二个数是与下一个客户间隔的时间（即下一个客户的到来时间=当前时间+间隔时间 当前时间就是第一个结点的第一个数据域）//
...
typedef struct event_list {
    ...
    int type; // -1表示用户达到 0-3表示用户从几号窗口离开//
...
N deleteWindow(Window window) {
    ...
    window->length--;
    if (window->length == 0) window->rear = window->front; // 如果弹出的是最后一个并不对rear处理的话 rear会成为野指针
    return node;
}
...
//            cout << "是新客户到来=请输入服务时间durTime和间隔时间interTime : ";
//            total_client++; // 总客户数++//
//            // 随机生成当前客户服务时间 和 下一个客户到来的间隔时间//
...
//            cout << "新客户进入queue" << flag << endl;
//            displayQueue(&queue[flag]);
//            cout << endl;
//            // 如果当前队列的长度为0 则将其离开事件插入事件链表//
//            if (min_length == 0) {
...
//            delete node;
//            // 删除该队列的第一个结点之后 如果当前队列的长度不为0 则需要将下一个结点的离开事件插入到事件链表中//
